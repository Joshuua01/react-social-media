...
const pictureSlice = createSlice({
  ...
  reducers: {
    ...
    removePicturesByAlbumId: (state, action: PayloadAction<number>) => {
      state.pictures = state.pictures.filter(
        (picture) => picture.albumId !== action.payload
      );
    },
  },
...
export const selectPictures = (state: { pictures: PictureState }) =>
  state.pictures.pictures;

export const { addPicture, removePicture, removePicturesByAlbumId } =
